...
if($_SERVER['REQUEST_METHOD'] == "POST")
{
    ...
    if($error == "") {
        ...
        if (!preg_match("#[0-9]+#", $user_password) && !preg_match("#[A-Z]+#", $user_password)) {
            ...
            $query = "insert into users (user_name,user_surname,user_email,user_password) values (:user_name,:user_surname,:user_email,:user_password)";
            $stmt = $connection->prepare($query);
            $stmt->execute($arr);
        }
